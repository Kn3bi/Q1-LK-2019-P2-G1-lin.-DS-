...
public class ViewController implements ActionListener, KeyListener, MouseListener, MouseMotionListener {
    ...
    private boolean isScenarioMode;

    /**
     * Erzeugt ein Objekt zur Kontrolle des Programmflusses.
     */
    public ViewController(boolean isScenarioMode){
        ...
        if(isScenarioMode){
            //new ScenarioController(1,this);   Da dies die Light-Version des Frameworks ist, wird der ScenarioController nicht bereitgestellt
        } else {
    ...
    private void createWindow(){
        ...
        if(!isScenarioMode) {
            ...
            drawFrame = new DrawFrame("Szenariomodus!", x, y, 1024, 1024, scenes.get(0).drawingPanel);
        }
     ...
     * @param i das gewünschte Objekt
     */
    public void register(Interactable i, int a){
        if (a < scenes.size() && i!=null){
            scenes.get(a).interactables.add(i);
        }
    }
     ...
     */
    public void remove(Drawable d, int sceneIndex){
        if ( sceneIndex < scenes.size() && d != null){
